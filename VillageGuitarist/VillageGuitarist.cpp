...
public:
    VillageGuitarist (const double sample_rate, const LV2_Feature *const *features);
    void connectPort (const uint32_t port, void* data_location);
    void run (const uint32_t sample_count);
};

VillageGuitarist::VillageGuitarist (const double sample_rate, const LV2_Feature *const *features) :
    midi_in_ptr (nullptr),
...
void VillageGuitarist::connectPort (const uint32_t port, void* data_location)
{
    switch (port)
    {
    case PORT_MIDI_IN:
...
void VillageGuitarist::run (const uint32_t sample_count)
{
...
void VillageGuitarist::update_position(const LV2_Atom_Object* obj)
{
...
{
    VillageGuitarist* g = nullptr;
    try 
    {
        g = new VillageGuitarist (sample_rate, features);
    } 
...
{
    VillageGuitarist* g = static_cast <VillageGuitarist*> (instance);
...
{
    "https://github.com/mmalessa/lv2/VillageGuitarist",
    instantiate,
    connect_port,
    activate,
    run,
